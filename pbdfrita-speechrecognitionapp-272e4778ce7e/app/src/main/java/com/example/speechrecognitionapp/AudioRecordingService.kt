...
class AudioRecordingService : Service() {
    ...
    // TODO kje the fuck se to kliče
    //treshold se razlikuje glede na situacijo:
    //->v kavarni:  2000-3000
    //-> uporabnik, ko je sam v tišini: 500-1000
    private fun isSilent1(audioBuffer: ShortArray, threshold: Short = 500): Boolean {
        var sum = 0.0
        for (sample in audioBuffer) {
            sum += Math.abs(sample.toInt()) // Pretvorite sample v Int pred klicanjem Math.abs
        }
        val average = sum / audioBuffer.size
        return average < threshold
    }
    private fun isSilent2(audioBuffer: ShortArray, threshold: Double = 1500.0): Boolean {
        var sum = 0.0
        for (sample in audioBuffer) {
            sum += sample.toDouble() * sample.toDouble()  // Kvadriranje vsakega vzorca
        }
        val energy = sum / audioBuffer.size  // Izračun povprečne energije
    ...
    //todo to je neki spremenjeno od mene, kliče isSilent
    private fun record() {
            ...
            while (totalSamplesRead < SAMPLE_RATE) {
                ...
                if (read != AudioRecord.ERROR_INVALID_OPERATION && read != AudioRecord.ERROR_BAD_VALUE) {
                   ...
                   if (isSilent2(audioBuffer)) {
                        ...
                        // Obdelajte avdio podatke, če ni tišina
    ...
    private fun loadAndPredict(mfccs: FloatArray) {
        ...
        if (axisLabels != null) {
            ...
            if (value!! > probabilityThreshold) {
                ...
                // Klic funkcije za posodabljanje štetja besed
                if (result != null) {
